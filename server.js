...
async function createServer() {
  ...
  let vite;
  let buildSPADocument;
  let homeRoutes, aboutRoutes, articlesRoutes, articleRoutes;
  ...
  if (!isProduction) {
    ...
    // Production mode - serve static assets and use pre-built SSR module
    const sirv = (await import("sirv")).default;
    app.use(sirv(path.resolve(__dirname, "dist/client"), { extensions: [] }));

    // Load pre-built SSR modules
