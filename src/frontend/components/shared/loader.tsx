import { useState, useEffect, useRef } from "react";

export const Loader = () => {
  const [secondValue, setSecondValue] = useState(0);
  const [thirdValue, setThirdValue] = useState(0);
  const [fourthValue, setFourthValue] = useState(3390);
  const phaseRef = useRef<
    | "animating-third"
    | "pause-between"
    | "animating-second"
    | "waiting"
    | "resetting"
  >("animating-third");

  useEffect(() => {
    const increment = 30; // Same speed for both stages
    const animate = () => {
      if (phaseRef.current === "animating-third") {
        setThirdValue((prev) => {
          if (prev >= 3390) {
            phaseRef.current = "pause-between";
            return 3390;
          }
          return prev + increment;
        });
        setFourthValue((prev) => {
          if (prev <= 0) {
            return 0;
          }
          return prev - increment;
        });
        setTimeout(animate, 1); // ~60fps
      } else if (phaseRef.current === "pause-between") {
        phaseRef.current = "animating-second";
        setTimeout(animate, 1000); // Wait 1 second between stages
      } else if (phaseRef.current === "animating-second") {
        setSecondValue((prev) => {
          if (prev >= 3390) {
            phaseRef.current = "waiting";
            return 3390;
          }
          return prev + increment;
        });
        setTimeout(animate, 1); // ~60fps
      } else if (phaseRef.current === "waiting") {
        phaseRef.current = "resetting";
        setTimeout(animate, 1000); // Wait 1 second
      } else if (phaseRef.current === "resetting") {
        setSecondValue(0);
        setThirdValue(0);
        setFourthValue(3390);
        phaseRef.current = "animating-third";
        setTimeout(animate, 1); // ~60fps
      }
    };

    const timeoutId = setTimeout(animate, 16);
    return () => clearTimeout(timeoutId);
  }, []);

  return (
    <div
      style={{
        display: "flex",
        justifyContent: "center",
        alignItems: "center",
        height: "100%",
        flexGrow: 1,
        width: "100%",
      }}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 100 100"
        preserveAspectRatio="xMidYMid"
        width="200"
        height="200"
        style={{
          shapeRendering: "auto",
          display: "block",
          background: "rgb(255, 255, 255)",
          height: "90px",
          width: "90px",
        }}
      >
        <g data-idx="1">
          <defs data-idx="2">
            <mask id="$id-mask" data-idx="3">
              <rect
                fill="#fff"
                height="90%"
                width="90%"
                y="5%"
                x="5%"
                data-idx="4"
              ></rect>
              <text
                font-family="arial"
                font-size="56"
                font-weight="900"
                dominant-baseline="central"
                text-anchor="middle"
                y="50%"
                x="50%"
                dy="0.01em"
                data-idx="5"
              >
                ZM
              </text>
            </mask>
          </defs>
          <g data-idx="6">
            <path
              d="M2.7 10.8c5.2-3 6.3-5.7 11.5-8.6c-5.7 4-6.8 10.8-10.5 16.7c6.3-6 12.9-12.1 20.3-16.7 c-6.5 5.2-9.7 12.1-15 18.5c7.3-4.8 13.1-12.3 20.4-17.2c-11.2 7.6-18.9 18.2-27.1 29C13.4 21.2 23.8 11.4 37.1 2.7 C24.1 12.3 12.2 26.9 2 39.5c14.1-11 29-25.8 43.2-36.8c-13.8 11.7-30.1 30.8-42.5 44C17.2 36.6 38.5 13.9 51.5 2 C37.1 15.4 17.7 39.1 3.3 52.5C21 38.1 40.3 18.3 58 4C39.3 20.9 21.3 40.3 2.7 57.2C24.9 40 45.7 20.1 67.9 2.9 C44.8 22.7 21.8 41.9 2.7 65.7C24.7 44.6 49.3 23.2 73.6 4.6C55.8 19.8 38.4 41.9 20.7 57.1c-5 4.3-15.4 12.6-17.6 18.8 C25.6 55.6 57.6 22.6 81.2 3.5C54.8 25.7 26.4 59 3.1 84.3c28-24.7 54.5-59.6 85.2-80.8C59.8 26.4 26.3 64 3.1 92.4 C29.4 69.5 67.6 26.8 93.9 4C65.5 31.7 28.5 75.4 2.3 105.2C33.8 72 66.9 30.4 104.9 4.9c-31.3 29.8-66 68-95 100.6 C42 76.3 70.8 43 105.4 16.8c-29.8 25.7-63.3 59.3-87.9 90c25.8-22 59-56.5 84.9-78.5c-25 21.4-56 53.8-77.7 78.5 c25-20.7 53.8-49.4 78.8-70.1c-25.2 21.6-49.3 47.4-71.7 72c24.7-19.4 46.9-44.2 71.7-63.5C81.4 63.1 61.6 86.6 44 108.8 c20-15.2 38.4-36.5 58.4-51.7c-14.6 11.1-34.8 37-47.2 50.6c13.6-9.1 34.1-30.2 46.9-40.4C90 75.8 74.8 95.7 65.4 107.1 c12.8-7.4 27.7-23.9 38.9-33.4c-12 9.3-20.8 21.8-31.6 32.6c11-6.6 21.6-15.8 30.9-24.6c-8 6.3-16.2 16.6-21.8 25.2 c8.8-5.6 17.8-10.3 21.4-15.9c-1.3 4.1-4.8 9.2-10.1 15.2c3.1-0.4 7.7-0.6 10.8-1"
              stroke-width="4"
              stroke="#ff5252"
              fill="none"
              mask="url(#$id-mask)"
              data-idx="7"
              stroke-dasharray={`0px, ${secondValue}px, ${thirdValue}px, ${fourthValue}px`}
            ></path>
          </g>
          <g data-idx="9"></g>
        </g>
      </svg>
    </div>
  );
};
